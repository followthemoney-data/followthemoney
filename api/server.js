...
async function fetchWithCache(key, fetchFn, ttlMinutes = 60) {
  const now = Date.now();
  if (memCache[key] && now - memCache[key].timestamp < ttlMinutes * 60 * 1000) {
    return memCache[key].data;
  }
  const data = await fetchFn();
  memCache[key] = { data, timestamp: now };
  return data;
}
...
app.get('/api/riksbank', async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: {
        m1: latest13(sort(history.m1)),
        m2: latest13(sort(history.m2)),
        m3: latest13(sort(history.m3))
      },
      source: 'live',
